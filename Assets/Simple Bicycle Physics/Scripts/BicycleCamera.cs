...
namespace SBPScripts
{
    public class BicycleCamera : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // Initialize rotation angles to 90 degrees to start the camera facing a specific direction
            wantedRotationAngle = 90.0f;
            currentRotationAngle = 90.0f;

            // Set the initial camera position based on the 90-degree rotation
            transform.position = target.position + Quaternion.Euler(0, 90.0f, 0) * new Vector3(0, height, -distance);
